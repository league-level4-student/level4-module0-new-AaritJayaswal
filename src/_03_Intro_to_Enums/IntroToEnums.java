...
public class IntroToEnums {
	...
	public static void main(String[] args) {
		...
		StatesOfMatter[] EnumsArray = StatesOfMatter.values();

		// 4. Ask the user for a state of matter.

		String matter = JOptionPane.showInputDialog(null, "Enter a state of matter: ");

		// 5. Iterate through the array and find what the user entered.
		// Hint: .name() or .toString
		StatesOfMatter input;

		for (int i = 0; i < EnumsArray.length; i++) {
			if (EnumsArray[i].toString().equalsIgnoreCase(matter)) {
				input = EnumsArray[i];
